  ...
  return (
    <Masonry breakpointCols={breakpointColumnsObj} className="masonry-grid mb-12" columnClassName="masonry-grid_column">
      {data.result.map((item) => (
        <div key={item.id} className="group relative overflow-hidden mt-2.5 rounded-xl bg-white dark:bg-black-b border dark:border-black-b hover:shadow-[0_10px_20px_1px_rgb(83,157,253,.1)]   cursor-pointer">
          {item.isTop && <PinBadge />}
          <Link href={`/article/${item.id}`}>
            ...
            <div className="py-2 px-4">
              <div className="mb-2 flex items-start gap-1.5">
                <h1 className="min-w-0 flex-1 text-black dark:text-white group-hover:text-primary line-clamp-2">{item.title}</h1>
              </div>
